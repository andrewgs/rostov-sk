				...
				<table class="table table-striped table-bordered">
					...
					<?php for($i=0;$i<count($events);$i++):?>
						<tr class="align-center">
							<td><nobr><?=$events[$i]['date'];?></nobr></td>
							<td>
								<p>
									<strong><?=$events[$i]['small_title'];?></strong> <br />
									<?=$events[$i]['small_text'];?>
								</p>
							</td>
							<td>
								<div id="params<?=$i;?>" style="display:none" data-nid="<?=$events[$i]['id'];?>"></div>
								<?=anchor('admin-panel/actions/news/edit/id/'.$events[$i]['id'],'<i class="icon-pencil"></i>',array('title'=>'Редактировать', 'class'=>'btn'));?>
